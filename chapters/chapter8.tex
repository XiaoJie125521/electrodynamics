...
先取 $\nu=0$。由 \eqref{eq:8.26} 得 $F^{i0}=E_{i}/c$，于是
\[
\partial_{\mu}F^{\mu0}
=
\partial_{i}F^{i0}
...
综上，四维方程 \eqref{eq:8.34} 与 \eqref{eq:8.36} 等价于四条三维 Maxwell 方程：
\begin{equation}
	\bm{\nabla}\cdot\bm{E}
	=
	\frac{\rho}{\varepsilon_{0}},
...
前者是动力学方程，后者来自 $F_{\mu\nu}$ 由势函数定义这一几何结构。对 $A_{\mu}$ 变分的电磁场作用量可推出非齐次 Maxwell 方程。
