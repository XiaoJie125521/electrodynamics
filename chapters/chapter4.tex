...
\begin{figure}
	\centering
	\includegraphics[width=0.5\linewidth]{figure/fig4.1}
	\caption[二维旋转变换]{二维旋转变换}
	\label{fig:4.1}
\end{figure}
...
\]
由 \eqref{eq:ch4-so2-rotation} 直接相乘可得
\begin{equation}
\label{eq:ch4-so2-composition}
g(\varphi_2)g(\varphi_1)=g(\varphi_1+\varphi_2).
\end{equation}
\eqref{eq:ch4-so2-composition} 说明连续两次平面转动仍是平面转动, 因而平面转动集合对变换复合封闭. 单位元为
\[
g(0)=I,
\]
逆元为
...
\]
平面转动群 $SO(2)$ 满足交换律,我们将在下面讲到这一类群被称为 Abel 群. 这个例子表明, 群乘法在变换群中就是“连续作变换”的复合运算. 

平面转动本身是 Abel 的, 但物理中更常见的是不同类型变换的复合. 例如在二维平面上, 平移和旋转都保持距离, 但二者的顺序通常不能交换. 记平移为
\[
T_{\bm{a}}:\bm{x}\mapsto \bm{x}+\bm{a},
\]
旋转为
\[
R:\bm{x}\mapsto R\bm{x}.
\]
则先平移再旋转得到
\[
R\,T_{\bm{a}}:\bm{x}\mapsto R\bm{x}+R\bm{a},
\]
而先旋转再平移得到
\[
T_{\bm{a}}\,R:\bm{x}\mapsto R\bm{x}+\bm{a}.
\]
除非 $R\bm{a}=\bm{a}$, 两者并不相同. 因此, 即使每一步都保持欧氏距离, 复合变换的顺序仍可能影响最终结果. 平面刚体运动群由平移和旋转共同组成, 它是一个典型的非 Abel 变换群. 

\begin{example}{平移与旋转的非交换性}{4,1}
在二维平面中, 设 $R_{\pi/2}$ 为逆时针旋转 $90^\circ$, $T_{\bm{a}}$ 为沿
\[
\bm{a}=(1,0)
\]
的平移. 比较 $R_{\pi/2}T_{\bm{a}}$ 与 $T_{\bm{a}}R_{\pi/2}$ 对点
\[
\bm{x}=(0,1)
\]
的作用. 
...
\item[封闭性] 对任意 $a,b\in G$, 都有
\[
ab\in G.
\]
\item[结合律] 对任意 $a,b,c\in G$, 都有
\[
(ab)c=a(bc).
\]
\item[单位元] 存在元素 $e\in G$, 使得对任意 $a\in G$, 都有
\[
ea=ae=a.
\]
\item[逆元] 对任意 $a\in G$, 存在 $a^{-1}\in G$, 使得
\[
aa^{-1}=a^{-1}a=e.
\]
\end{description}
...
\[
\bm{r}\mapsto \bm{r}'=R\bm{r}.
\]

在场论中, 群作用通常同时包含两部分: 一是自变量的变换, 二是场分量本身的变换. 以矢量场 $\bm{A}(\bm{r})$ 为例, 旋转后可写为
\[
\bm{A}'(\bm{r}')=R\bm{A}(\bm{r}),
\qquad
\bm{r}'=R\bm{r}.
\]
这表示变换后的场值在新点 $\bm{r}'$ 处等于原场值在旧点 $\bm{r}$ 处经过矢量表示矩阵 $R$ 作用后的结果. 

也可以在同一个坐标点比较变换前后的场构型, 此时写成
\[
\bm{A}'(\bm{r})=R\bm{A}(R^{-1}\bm{r}).
\]
...
则要求长度在变换下不变, 即
\[
\bm{v}'^Tg\bm{v}'=\bm{v}^Tg\bm{v}.
\]
代入 $\bm{v}'=A\bm{v}$, 得到
\[
\bm{v}^TA^TgA\bm{v}=\bm{v}^Tg\bm{v}.
\]
由于该式对任意 $\bm{v}$ 成立, 必须有
\begin{equation}
\label{eq:ch4-metric-preserving}
A^TgA=g.
\end{equation}
因此, 不同几何结构中的“旋转”可以看作保持相应度规的变换. 若 $g=I$, 则 \eqref{eq:ch4-metric-preserving} 给出欧氏正交群. 这个观点把长度、角度与变换群之间的关系清楚地联系起来. 

这也是几何中常说的“不变量观点”. 一种几何结构可以由它在某个变换群下保持不变的量来刻画. 欧氏几何的核心不变量是正定内积
\[
\bm{v}\cdot\bm{v}=v^iv^j\delta_{ij},
\]
...
\subsection{一般线性群与特殊线性群}

$n$ 维实一般线性群定义为
\begin{equation}
\label{eq:ch4-gl-definition}
GL(n,\mathbb{R})
=
\{A\in M_n(\mathbb{R})\mid \det A\neq 0\}.
\end{equation}
\eqref{eq:ch4-gl-definition} 包含所有可逆实矩阵, 群运算是矩阵乘法. 单位元是单位矩阵 $I$, 每个 $A\in GL(n,\mathbb{R})$ 都有逆矩阵 $A^{-1}$. 

特殊线性群定义为
\begin{equation}
\label{eq:ch4-sl-definition}
SL(n,\mathbb{R})
=
...
\det(AB)=\det A\det B,
\]
所以 \eqref{eq:ch4-sl-definition} 中的 $SL(n,\mathbb{R})$ 在矩阵乘法下封闭. 该群表示保持有向体积不变的线性变换. 

\subsection{正交群与特殊正交群}

$n$ 维正交群定义为
\begin{equation}
\label{eq:ch4-o-definition}
O(n)
=
\{R\in GL(n,\mathbb{R})\mid R^TR=I\}.
\end{equation}
等价地, \eqref{eq:ch4-o-definition} 表示 $R$ 保持欧氏度规不变: 
\[
R^T\delta R=\delta.
\]
若矢量变换为
...
特殊正交群定义为
\begin{equation}
\label{eq:ch4-so-definition}
SO(n)
=
\{R\in O(n)\mid \det R=1\}.
\end{equation}
其中 \eqref{eq:ch4-so-definition} 给出的 $SO(3)$ 是三维空间中真正的旋转群, 不包含镜面反射. 在物理中, $SO(3)$ 描述空间各向同性. 

\subsection{酉群与特殊酉群}

在复线性空间中, 保持厄米内积的矩阵构成酉群: 
\begin{equation}
\label{eq:ch4-u-definition}
U(n)
=
...
若进一步要求行列式为 $1$, 得到特殊酉群: 
\begin{equation}
\label{eq:ch4-su-definition}
SU(n)
=
\{U\in U(n)\mid \det U=1\}.
\end{equation}
在量子力学和场论中, 酉群非常重要. \eqref{eq:ch4-u-definition} 中的 $U(1)$ 与电磁相位对称性有关, \eqref{eq:ch4-su-definition} 中的 $SU(2)$ 和 $SU(3)$ 分别出现在弱相互作用和强相互作用的规范结构中. 本章只把它们作为重要矩阵群列出, 具体物理意义将在后续章节讨论. 

\subsection{\texorpdfstring{$SO(2)$ 与平移群}{SO(2) 与平移群}}
...
\begin{solution}
对小参数 $a,b$ 作 Taylor 展开,
\[
f(x-a,y-b)
=
f(x,y)-a\partial_x f-b\partial_y f+O(a^2,b^2,ab).
\]
因此
\[
T_{(a,b)}
=
I-a\partial_x-b\partial_y+\cdots.
\]
若采用量子力学中常见的厄米生成元约定, 平移生成元可写为
\[
P_x=-i\hbar\partial_x,
\qquad
P_y=-i\hbar\partial_y.
\]
在这个约定下,
\[
T_{(a,b)}
...
\section{\texorpdfstring{三维旋转群 $SO(3)$}{三维旋转群 SO(3)}}

三维旋转矩阵 $R$ 满足
\begin{equation}
\label{eq:ch4-so3-condition}
R^TR=I,
\qquad
\det R=1.
\end{equation}
...
R\in SO(3).
\]
\eqref{eq:ch4-so3-condition} 是三维旋转矩阵的定义性条件. 如果位置矢量变换为
\[
\bm{r}'=R\bm{r},
\]
那么任意两个矢量的内积保持不变. 设
...
[L_z,L_x]=L_y.
\end{equation}
因此三维旋转的顺序会影响最终结果. \eqref{eq:ch4-so3-real-algebra} 与平面旋转 $SO(2)$ 的 Abel 性形成对比. 
...
\]
这表示先后用三个角参数描述一个一般旋转. 不同教材可能采用不同的轴序和主动、被动约定, 但共同点是: $SO(3)$ 是三参数李群, 而三个参数背后的局部结构由 \eqref{eq:ch4-so3-real-algebra} 中的生成元对易关系决定. 

\begin{example}{无穷小正交变换的生成元必须反对称}{4,3}
设三维无穷小线性变换为
\[
R=I+\varepsilon,
\]
其中 $\varepsilon$ 只保留一阶小量. 若 $R$ 是正交变换, 求 $\varepsilon$ 满足的条件, 并写出最一般形式. 
\begin{solution}
正交条件为
\[
R^TR=I.
...
\[
(I+\varepsilon^T)(I+\varepsilon)
=
I+\varepsilon^T+\varepsilon
=I.
\]
因此
\[
...
\end{pmatrix}
=
\delta\theta_xL_x+\delta\theta_yL_y+\delta\theta_zL_z.
\]
所以 $SO(3)$ 的无穷小旋转只有三个独立参数, 正好对应绕三条坐标轴的三个生成元. 
\end{solution}
\end{example}

\subsection{标量、矢量与张量的旋转变换}
...
满足
\begin{equation}
\label{eq:ch4-representation-condition}
...
D(e)=I,
\]
则称 $D$ 是群 $G$ 在空间 $V$ 上的一个表示. 这里 $V$ 称为表示空间. \eqref{eq:ch4-representation-condition} 表明表示必须保持群乘法结构. 
...
下, 标量场满足
\[
\varphi'(\bm{x}')=\varphi(\bm{x}).
\]
矢量场满足
\[
\bm{A}'(\bm{x}')=R\bm{A}(\bm{x}),
\]
分量形式为
\[
A'^i(\bm{x}')=R^i{}_jA^j(\bm{x}).
\]
二阶张量场满足
\[
T'^{ij}(\bm{x}')
=
R^i{}_kR^j{}_lT^{kl}(\bm{x}).
\]
...
在单位元附近展开群元,
\begin{equation}
\label{eq:ch4-lie-group-expansion}
g(\alpha)
=
...
\right|_{\alpha=0}
\]
称为李群的生成元. 生成元描述的是从单位元出发的无穷小方向. \eqref{eq:ch4-lie-group-expansion} 对矩阵群来说就是单位元处切空间中的一阶展开. 
...
\begin{equation}
\label{eq:ch4-lie-algebra-structure}
[X_i,X_j]=C_{ij}{}^kX_k.
\end{equation}
常数 $C_{ij}{}^k$ 称为结构常数, 它们刻画李群在单位元附近的非交换结构. \eqref{eq:ch4-lie-algebra-structure} 这组对易关系就是李代数. 
...
\begin{equation}
\label{eq:ch4-finite-generator}
D(g(\alpha))
=
\exp\left(-\frac{i}{\hbar}\alpha\hat{Q}\right).
\end{equation}
这里采用量子力学和场论中常见的约定: 生成元取为厄米算符, \eqref{eq:ch4-finite-generator} 的指数中显式出现 $-i/\hbar$. 如果采用数学中的实反对称矩阵生成元, 则公式中不会出现该因子. 两种约定等价, 但符号形式不同. 
...
\begin{equation}
\label{eq:ch4-physical-lie-algebra}
[\hat{T}_a,\hat{T}_b]
=
i\hbar f_{ab}{}^c\hat{T}_c,
\end{equation}
则这些生成元构成李代数. 其中 $f_{ab}{}^c$ 称为结构常数. \eqref{eq:ch4-physical-lie-algebra} 是物理厄米生成元约定下的李代数写法. 
...
\subsection{\texorpdfstring{$SO(3)$ 的李代数}{SO(3) 的李代数}}
...
\begin{equation}
\label{eq:ch4-angular-momentum-algebra}
[\hat{J}_i,\hat{J}_j]
=
...
\]
这与 \eqref{eq:ch4-angular-momentum-algebra} 所采用的物理厄米生成元约定并不矛盾, 只是作用空间和符号约定不同. 
...
\subsection{\texorpdfstring{$SU(2)$ 与 Pauli 矩阵简介}{SU(2) 与 Pauli 矩阵简介}}
...
以及
\begin{equation}
\label{eq:ch4-pauli-anticommutator}
\{\sigma_i,\sigma_j\}
=
2\delta_{ij}I.
\end{equation}
由对易关系 \eqref{eq:ch4-pauli-commutator} 和反对易关系 \eqref{eq:ch4-pauli-anticommutator} 合起来可得
\begin{equation}
\label{eq:ch4-pauli-product}
\sigma_i\sigma_j
=
\delta_{ij}I+i\varepsilon_{ijk}\sigma_k.
\end{equation}
因此对任意三维向量 $\bm{A}$ 和 $\bm{B}$,
\begin{equation}
\label{eq:ch4-pauli-vector-identity}
(\bm{\sigma}\cdot\bm{A})(\bm{\sigma}\cdot\bm{B})
=
\bm{A}\cdot\bm{B}
+i\bm{\sigma}\cdot(\bm{A}\times\bm{B}).
\end{equation}
\eqref{eq:ch4-pauli-product} 推出的 \eqref{eq:ch4-pauli-vector-identity} 在自旋计算、电磁相互作用和多分量波动方程中都会反复出现. 

\begin{example}{自旋态绕 $z$ 轴旋转}{4,4}
自旋 $1/2$ 态绕 $z$ 轴旋转角度 $\theta$ 的变换为
\[
U_z(\theta)
=
\exp\left(-\frac{i}{2}\theta\sigma_3\right).
\]
...
-I.
\]
因此自旋 $1/2$ 态矢量在空间旋转 $2\pi$ 后不是回到原来的向量, 而是整体变号:
\[
...
则有
\begin{equation}
\label{eq:ch4-spin-algebra}
[\hat{S}_i,\hat{S}_j]
=
i\hbar\varepsilon_{ijk}\hat{S}_k.
\end{equation}
\eqref{eq:ch4-spin-algebra} 与角动量代数 \eqref{eq:ch4-angular-momentum-algebra} 相同. 因此, $SU(2)$ 给出了旋转群在自旋空间中的重要表示. 更深入地, $SU(2)$ 是 $SO(3)$ 的双覆盖: $SU(2)$ 中相差一个整体负号的两个群元素对应于同一个三维空间旋转. 这个事实是自旋 $1/2$ 粒子在旋转 $2\pi$ 后态矢量变号的群论来源. 

\section{协变方程的群论理解}

设群 $G$ 作用在一组场 $\varphi$ 上, 某个方程写成
\begin{equation}
\label{eq:ch4-covariant-equation}
\mathcal E[\varphi]=0,
\end{equation}
若对群中任意变换 $g$, 变换后的场 $\varphi'=g\cdot\varphi$ 仍满足同样形式的方程
\begin{equation}
\label{eq:ch4-transformed-covariant-equation}
\mathcal E[\varphi']=0,
\end{equation}
则称该方程在群 $G$ 下协变. 也就是说, \eqref{eq:ch4-covariant-equation} 在群变换后仍保持为 \eqref{eq:ch4-transformed-covariant-equation} 的同一形式. 这里的重点不是某个分量数值是否不变, 而是方程两边在群作用下是否仍属于同一类对象, 并以同样规则变换. 
