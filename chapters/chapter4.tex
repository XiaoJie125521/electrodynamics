...
平面转动是最简单的连续变换群之一. 设 $g(\phi)$ 表示平面内逆时针转动角度 $\phi$, 其矩阵表示为
\[
g(\phi)
=
\begin{pmatrix}
...
\end{pmatrix}.
\]
若先转动角度 $\phi_1$, 再转动角度 $\phi_2$, 则总变换为
\[
g(\phi_2)g(\phi_1).
\]
直接相乘可得
\[
g(\phi_2)g(\phi_1)=g(\phi_1+\phi_2).
\]
...
由于
\[
g(\phi_2)g(\phi_1)=g(\phi_1)g(\phi_2),
\]
平面转动群 $SO(2)$ 是 Abel 群. 这个例子表明, 群乘法在变换群中就是“连续作变换”的复合运算. 
...
设群 $G$ 作用在一组场 $\Phi$ 上, 某个方程写成
\[
\mathcal E[\Phi]=0,
\]
若对群中任意变换 $g$, 变换后的场 $\Phi'=g\cdot\Phi$ 仍满足同样形式的方程
\[
\mathcal E[\Phi']=0,
\]
